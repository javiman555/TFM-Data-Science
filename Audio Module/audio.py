...
results_dict = {
    ...
    'ECE after temperature': ece.item(),
    'Loss after temperature': val_loss  # Assuming you want to store the final validation loss
}
...
for epoch in range(num_epochs_stage2):
    train_loss = train_model(model, train_loader, criterion, optimizer, device)
    val_loss, val_accuracy = evaluate_model(model, val_loader, criterion, device)
    print(f'Stage 2 - Epoch {epoch+1}, Validation Loss: {val_loss:.4f}, Validation Accuracy: {val_accuracy:.4f}')
...
results_dict = {
    'Test Accuracy': test_accuracy,
    'ECE after TST': ece.item(),
    'Loss after TST': val_loss  # Assuming you want to store the final validation loss
}
