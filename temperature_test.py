...
validation_data, validation_target = torch.load(os.path.join(save_dir, 'validation_data.pth'))
...
model.load_state_dict(torch.load(os.path.join(save_dir, 'dan_model.pth')))
model.to(device)
# Apply temperature scaling
scaled_model = ModelWithTemperature(model)
scaled_model.set_temperature(valid_loader)

# Evaluate the model with temperature scaling on the validation set
logits_list = []
labels_list = []
scaled_model.eval()
with torch.no_grad():
...
print('Expected Calibration Error (ECE) after Temperature Scaling:', ece.item())
