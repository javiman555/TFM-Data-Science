...
print('Expected Calibration Error (ECE) before TST:', ece.item())

# Stage 2: Freeze Feature Extractor and Reinitialize FC Layers
for param in model.parameters():
    param.requires_grad = False

model.fc_out = nn.Linear(hidden_size, output_size).to(device)

# Reinitialize optimizer for the reinitialized FC layer
...
# Save trained model
torch.save(model.state_dict(), os.path.join(save_dir, 'dan_model_tst.pth'))
