...
def train_model(model, loader, criterion, ece_criterion, optimizer, device, lambda_ece=0.1):
    model.train()
    running_loss = 0.0
    for inputs, labels in loader:
        ...
        loss = loss_ce + lambda_ece * loss_ece
        loss.backward()
        optimizer.step()
        running_loss += loss.item() * inputs.size(0)
    return running_loss / len(loader.dataset)

def evaluate_model(model, loader, criterion, ece_criterion, device, lambda_ece=0.1):
    model.eval()
    running_loss = 0.0
    with torch.no_grad():
        for inputs, labels in loader:
...
learning_rate = 0.001
num_epochs = 10
lambda_ece = 0.1  # Weight for ECE in the loss function
...
for epoch in range(num_epochs):
    train_loss = train_model(model, train_loader, criterion, ece_criterion, optimizer, device, lambda_ece)
    val_loss = evaluate_model(model, validation_loader, criterion, ece_criterion, device, lambda_ece)
...
# Save trained model
torch.save(model.state_dict(), os.path.join(save_dir, 'dan_model_calibrated.pth'))
